...
public class Config
{
    ...
    internal static HashSet<string> blacklist = new();
    internal static Dictionary<string, double> yields = new();
    ...
    static Config()
    {
        ...
        {
            if (JsonUtil.TryFileToObject(file, out FileData data))
            { data.Apply(); }
        }
    ...
    {
        return yields.TryGetValue(techType.AsString(), out double yeild) ? (float)yeild : 1;
    }
